...
class GeneticOptimizer:
    ...
    def verify_checkpoint(self):
        ...
        os.makedirs(os.path.join(RUNS_FOLDER, 'enemy_' + str(ENEMY)), exist_ok=True)
        ...
        checkpoint_path = os.path.join(RUNS_FOLDER, 'enemy_' + str(ENEMY), self.checkpoint)
...
if __name__ == "__main__":
    ...
    if not optimizer.parallel:
        ...
        best_individual_path = os.path.join(RUNS_FOLDER, "enemy_" + str(ENEMY), "best_individual.txt")
        np.savetxt(best_individual_path, best_individual["weights_and_biases"])

        logbook = optimizer.getLogbook()
        logbook_path = os.path.join(RUNS_FOLDER, "enemy_" + str(ENEMY), "run_" + str(N_RUN) + ".csv")
